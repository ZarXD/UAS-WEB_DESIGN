    ...
    // Mobile Menu
            ...
            animes.forEach(anime => {
                const animeCard = `
                    <div class="anime-card bg-gray-800 rounded-lg overflow-hidden shadow-lg cursor-pointer hover:transform hover:scale-105 transition-transform" data-id="${anime.id}">
                        <img 
                            src="${anime.attributes.posterImage?.large || 'https://via.placeholder.com/300'}" 
        ...
        // Fungsi untuk rekomendasi anime
        async function fetchRecommendations() {
            try {
                const response = await fetch(
                    'https://kitsu.io/api/edge/anime?sort=-averageRating&page[limit]=6'
                );
        ...
        function displayRecommendations(animes) {
            ...
            animes.forEach(anime => {
                const recommendationCard = `
                    <div class="anime-card cursor-pointer flex-shrink-0 w-48 bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:transform hover:scale-105 transition-transform" data-id="${anime.id}">
                        <img 
                            src="${anime.attributes.posterImage?.large || 'https://via.placeholder.com/150'}" 
                            alt="${anime.attributes.canonicalTitle}"
                ...
                `;
                container.innerHTML += recommendationCard;
            });
        }
